...
| `preflight`       | Validates `--format`, `--coverage-format`, and `--fail-on-risk`; compares `--base` and `--head` or, with `--worktree`, compares `--base` against staged, unstaged, and untracked worktree changes; imports optional Go/LCOV coverage from flags or an existing configured artifact; imports bounded optional analyzer findings for changed files unless `--no-external-tools` is set; adds recent personal pattern checks when history is available; writes V2 preflight artifacts; exits nonzero for risk only when configured. | Go command test, dogfood smoke, action contract test.   |
...
- `preflight.json` is V2. It includes structured changed files, additions and
  ...
  is available. `--worktree` includes staged, unstaged, and untracked
  non-ignored files. Missing coverage is `unknown`, not `uncovered`.
