...
func TestMarkdownIncludesPrivateExplainabilityDeepDivesAndSetup(t *testing.T) {
	analysis := signals.AnalysisReport{
		...
		FollowUp: signals.FollowUpComparison{
			Status:              "available",
			PreviousGeneratedAt: time.Date(2025, 12, 25, 0, 0, 0, 0, time.UTC),
			CurrentGeneratedAt:  time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
			Summary:             "Since the last report, 1 improved, 1 got worse.",
			Improved:            []signals.Finding{{Label: "Test evidence improved", Evidence: "Source changes with test evidence changed from 0.0% in the last report to 100.0% now.", Confidence: signals.ConfidenceMedium}},
			Regressed:           []signals.Finding{{Label: "Fix/revert-like churn increased", Evidence: "Fix/revert-like churn rate changed from 0.0% in the last report to 20.0% now.", Confidence: signals.ConfidenceMedium, NextAction: "Inspect repeat fix patterns."}},
			Resolved:            []signals.Finding{{Label: "Large changes create review risk", Evidence: "This weakness appeared in the previous report but is not present in the current top weakness map.", Confidence: signals.ConfidenceLow}},
			Persistent:          []signals.Finding{{Label: "No-test pattern", Evidence: "This weakness appears in both the previous and current reports.", Confidence: signals.ConfidenceMedium, NextAction: "Add nearby checkout regression tests."}},
			NextAction:          "Inspect repeat fix patterns.",
			Confidence:          signals.ConfidenceMedium,
		},
	...
	for _, want := range []string{
		...
		"## Since Last Report",
		"Since the last report, 1 improved, 1 got worse.",
		"Got worse:",
		"Fix/revert-like churn increased",
		"Best next move: Inspect repeat fix patterns.",
		"## Confidence Setup",
...
func TestPublicSafeAnalysisRedactsDeepDivesAndCoverage(t *testing.T) {
	...
	analysis := publicsafe.Analysis(signals.AnalysisReport{
		...
		FollowUp: signals.FollowUpComparison{
			Status:     "available",
			Summary:    "Since the last report, private path " + privatePath + " improved.",
			Improved:   []signals.Finding{{Label: "Fixed " + commitSHA[:8], Evidence: "Improved " + privatePath, Confidence: signals.ConfidenceMedium}},
			Regressed:  []signals.Finding{{Label: "Regression " + commitSHA, Evidence: "Risk in " + privatePath, Confidence: signals.ConfidenceMedium, NextAction: "Add tests for " + privatePath}},
			NextAction: "Keep checking " + privatePath,
			Confidence: signals.ConfidenceMedium,
		},
