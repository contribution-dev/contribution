...
| `analyze`         | Analyzes `--repo` or the current repo, always writes canonical `analysis.json`, respects `--format`, optionally imports Go/LCOV coverage, supports explicit `--github-token gh`, imports optional analyzer findings when tools are available, and continues without optional tools or GitHub metadata. | Go analysis tests, dogfood smoke, real-repo dogfood.    |
...
| `preflight`       | Validates `--format`, `--coverage-format`, and `--fail-on-risk`; compares `--base` and `--head`; imports optional Go/LCOV coverage; adds recent personal pattern checks when history is available; writes V2 preflight artifacts; exits nonzero for risk only when configured.                         | Go command test, dogfood smoke, action contract test.   |
...
- `analysis.json` includes `coverage`, `analyzer_findings`, `deep_dives`, and
  `setup_actions`. `coverage` summarizes explicitly imported Go/LCOV coverage,
  including the lowest-coverage files. `analyzer_findings` stores normalized
  optional-tool findings without raw code. `deep_dives` explains high-churn and
  source-without-test patterns for the private receipt. `setup_actions` gives
  concrete commands that would raise confidence.
- `preflight.json` is V2. It includes structured changed files, additions and
  ...
  changed-line coverage, rubric items, and optional `personal_context` when
  recent local history is available. Missing coverage is `unknown`, not
  `uncovered`.
