...
- Commands must be deterministic about stdout, stderr, exit status, and output
  files.
- Successful `analyze` and `probe` runs print concise terminal summaries with
  the main result, the top-read headline, top next-PR actions, follow-up
  movement when available, capped unavailable-signal notes, and only the
  artifact paths actually written for the selected format. Successful
  `preflight` runs print the current-diff risk receipt, capped unavailable
  notes, and only the artifact paths written for the selected format.
...
- Local AI work evidence discovery is explicit opt-in through the `evidence`
  command group. Default evidence bundles are derived only and must not contain
  raw prompts, raw model outputs, transcript JSONL, raw diffs, terminal logs,
  source code, secrets, environment values, private keys, credential URLs, or
  local session file paths. Hosted upload remains disabled until the CLI
  consumes a finalized website receiving contract.
- Successful `analyze`, `probe`, `report`, and `export-profile` output should
  include a concise public-safe share-card handoff: the rendered share-card
  title/subtitle/highlights, `https://contribution.dev/share`, and the
  `profile.export.json` and `share-card.json` paths to upload in the website
  tool. This is a handoff only; the CLI must not upload or host the artifacts.
...
| `analyze`         | Analyzes `--repo` or the current repo, always writes canonical `analysis.json`, readiness/source/attribution collector artifacts, profile/share artifacts, and tooling data; respects `--format`; optionally imports Go/LCOV coverage from flags or an existing configured artifact; supports explicit `--github-token gh`, `--github-token env:NAME`, env-name, or literal token sources without reading ambient tokens by default; imports optional analyzer findings when tools are available on `PATH` or explicitly trusted repo-local tools are enabled for a local repo; can import metadata-only agent artifacts with explicit opt-in; compares recent and prior local history windows; and continues without optional tools or GitHub metadata.                                                                   | Go analysis tests, dogfood smoke, real-repo dogfood.    |
| `probe`           | Runs a JSON-only local collector for web-app import. It writes `analysis.json`, `collector.bundle.json`, `source-coverage.json`, `attribution-readiness.json`, profile/share artifacts, and `tooling.json`; does not upload; defaults `analysis.json` to public-safe output while collector artifacts remain public-safe by construction; supports optional GitHub metadata only with an explicit `--github-token` source; and requires `--include-agent-artifacts` before reading any explicit `--agent-artifact` metadata path.                                                                                                                                                                                                                                                                                          | Go command test and dogfood smoke.                      |
| `evidence`        | `evidence preview` scans Claude Code and Codex CLI session artifacts only after the user runs the command, filters sessions to the current local repo where possible, and prints sources scanned, sessions found/linked/skipped, fields extracted/redacted/blocked, and what would be exported. `evidence export` writes an offline `ai-work-evidence.bundle.json` plus `redaction-receipt.json` under the report output root and does not contact hosted services; remote Git URLs are rejected to preserve offline mode. `evidence doctor` checks source directory availability without reading sessions. `evidence upload` is disabled until the CLI consumes a finalized website receiving contract.                                                                                                                   | Go command test and package tests.                      |
...
| `report`          | Requires `--input`, validates `--format`, regenerates report/profile/share artifacts, honors `--public-safe`, and prints the public-safe share-card handoff.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Go command test and dogfood smoke.                      |
| `export-profile`  | Requires `--input` and `--output`, writes only public-safe `profile.export.json` and `share-card.json`, prints the public-safe share-card handoff, and never writes report artifacts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Go command test and dogfood smoke.                      |
...
| `preflight`       | Validates `--format`, `--coverage-format`, and `--fail-on-risk`; compares `--base` and `--head` or, with `--worktree`, compares `--base` against staged, unstaged, and untracked worktree changes; when `--base` is omitted, uses `project.default_branch` from `.contribution.yml`, or `main` when no config sets it; imports optional Go/LCOV coverage from flags or an existing configured artifact; with `--run-coverage`, runs configured `coverage.command` without shell expansion before importing coverage; imports bounded optional analyzer findings for changed files from `PATH` tools or explicitly trusted repo-local tools unless `--no-external-tools` is set; adds recent personal pattern checks when history is available; writes V2 preflight artifacts; exits nonzero for risk only when configured. | Go command test, dogfood smoke, action contract test.   |
...
- `analysis.json` includes `coverage`, `top_read`, `analyzer_findings`,
  `trends`, `follow_up`, `deep_dives`, and `setup_actions`. `coverage`
  summarizes explicitly imported Go/LCOV coverage, including the
  lowest-coverage files. `top_read` is an additive deterministic first-read
  summary with a headline, evidence-backed findings, a next-PR plan, and
  confidence. `analyzer_findings` stores normalized optional-tool findings
  without raw code. `trends` compares the recent local-history window with the
  immediately prior window for test evidence, large changes, fix/revert-like
  churn, risky untested changes, and high-churn concentration. `follow_up`
  compares the current report with the latest prior local `analysis.json` under
  the same output root and records improved, regressed, resolved, and
  persistent patterns. `deep_dives` explains high-churn and source-without-test
  patterns for the private report. `setup_actions` gives concrete commands that
...
- `collector.bundle.json` is public-safe by construction. It includes schema
  version, generated time, redacted repo metadata, local git summary,
  `top_read`, tooling, agentic readiness, source coverage, data gaps,
